# 导包
from api.login import LoginAPI
from api.course import CourseAPI
# import pytest
#断言是为了检查测试结果
# 创建测试类
class TestSelectCourse:
    # 初始化
    token = None

    # 前置处理
    # @pytest.fixture(autouse=True)
    def setup_class(self):
        # 实例化接口对象
        self.login_api = LoginAPI()
        self.select_api = CourseAPI()

        # 获取验证码
        res_v = self.login_api.get_verify_code()
        login_data = {
            "username": "admin",
            "password": "HM_2023_test",
            "code": 2,
            "uuid": res_v.json().get("uuid")
        }
        res_l = self.login_api.login(test_data=login_data)
        # 提取登录成功之后的token数据并保存在类的属性中
        TestSelectCourse.token = res_l.json().get("token")
        print(TestSelectCourse.token)
    # 后置处理
    def teardown_class(self):
        pass
    # 查询课程成功
    def test01_select_course_success(self):
        str = "?name=接口测试001"

        res_s = self.select_api.selectcourse(test_data=str, token=TestSelectCourse.token)
        print(res_s.json())
        # 断言响应状态码
        assert 200 == res_s.status_code
        # 断言指定文字
        assert '成功' in res_s.text
        # 断言json返回数据code值
        assert 200 == res_s.json().get("code")


    # 查询课程失败（用户未登录）
    def test02_select_course_fail(self):
        str = "?name=测试开发提升课01"
        res_s = self.select_api.selectcourse(test_data=str, token="xxx")
        print(res_s.json())

        # 断言响应状态码
        assert 200 == res_s.status_code
        # 断言指定文字
        assert '失败' in res_s.text
        # 断言json返回数据code值
        assert 401 == res_s.json().get("code")